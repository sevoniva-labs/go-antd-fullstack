...
func TestServerRequiresMatchingTokenForSessionWrites(t *testing.T) {
	tests := []struct {
		...
		{name: "matching session token", principal: domain.Principal{Type: "SESSION"}, header: testHeader{"Cookie": {"forge_csrf=secret"}, "X-CSRF-Token": {"secret"}}},
		{name: "missing session token", principal: domain.Principal{Type: "SESSION"}, header: testHeader{"Cookie": {"forge_csrf=secret"}}, wantError: true},
