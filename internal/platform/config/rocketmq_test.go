...
func TestValidateRequiresCompleteRocketMQConfiguration(t *testing.T) {
	tests := []struct {
		...
		{name: "access key", want: "FORGE_ROCKETMQ_ACCESS_KEY", prepare: func(m *Messaging) { m.RocketMQAccessKey = "" }},
		{name: "secret key", want: "FORGE_ROCKETMQ_SECRET_KEY", prepare: func(m *Messaging) { m.RocketMQSecretKey = "" }},
	}
