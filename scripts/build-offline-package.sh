...
release_status=Not certified until target signing, air-gapped install, upgrade, rollback, and recovery evidence is attached
EOF

if [[ -n "${OFFLINE_SIGNATURE_FILE:-}" ]]; then
  [[ -s "$OFFLINE_SIGNATURE_FILE" ]] || {
    echo "OFFLINE_SIGNATURE_FILE is not a readable non-empty file" >&2
    exit 1
  }
  cp "$OFFLINE_SIGNATURE_FILE" "$STAGING/release-signature"
fi
