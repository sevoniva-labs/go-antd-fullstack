...
4. 保持生产 `auto_migrate=false`；迁移只由 `forge-migrate` 一次性任务执行。
...
```text
完整读取并严格执行仓库 AGENTS.md、.agents/skills/forge-banking-scaffold/SKILL.md、docs/ai-engineering-governance.md 和 docs/velora-backend-adoption-plan.md。
