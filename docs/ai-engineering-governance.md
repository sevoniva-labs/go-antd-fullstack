...
Codex uses `AGENTS.md` and `.agents/skills`; Claude imports `AGENTS.md` through `CLAUDE.md` and uses `.claude/skills`; Cursor always loads `.cursor/rules/forge-banking-scaffold.mdc`. `scripts/check-ai-governance.sh` prevents drift. No file can control a client that deliberately ignores project instructions, so supported clients use both automatic loading and a CI rejection gate.
