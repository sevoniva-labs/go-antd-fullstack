...
- 服务端必须同时设置 `FORGE_WEB_CSP_WUJIE_ENABLED=true` 和有效的 `FORGE_WEB_CSP_WUJIE_APPROVAL_REF`。
...
- Origin 必须进入 `FORGE_WEB_CSP_FRAME_SOURCES` 精确白名单，不允许通配符。
- 宿主对远程健康检查或 API 的访问必须单独进入 `FORGE_WEB_CSP_CONNECT_SOURCES`。
