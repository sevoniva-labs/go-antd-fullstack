...
- `scripts/test-mysql-backup-restore-contract.sh` provides the corresponding disposable MySQL backup/restore path: create a contract database, run `mysqldump --single-transaction`, recreate the database, restore, and verify the row. Run it with the same temporary credentials, `FORGE_MYSQL_BACKUP_PORT`, and immutable `FORGE_MYSQL_IMAGE`; optionally set `FORGE_MYSQL_BACKUP_EVIDENCE_FILE`. This remains local single-instance backup evidence, not PITR, HA, cross-site recovery, or regulatory certification.
- 2026-08-21: the exact domestic mirror image `docker.m.daocloud.io/library/mysql@sha256:b3b90af2a6552ae30c266fdb7d5dd55f3afb72404bb78d37fe8a23eb857fd3fb` passed the new backup/restore contract. The run created a disposable database, exported with `mysqldump --single-transaction`, recreated and restored it, and verified the payload; the generated evidence JSON was valid. This is local single-instance evidence only.
