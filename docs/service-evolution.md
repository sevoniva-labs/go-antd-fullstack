...
| 层次 | 路径 | 依赖边界 |
|---|---|---|
| Proto 契约 | `api/proto/forge/v1/reference_settlement.proto` | 生成 HTTP、gRPC 和 OpenAPI |
...
FORGE_EXAMPLE_SETTLEMENT_ORGANIZATION_ID=org-1 \
FORGE_EXAMPLE_SETTLEMENT_TOKEN_SHA256="$DIGEST" \
...
```text
FORGE_EXAMPLE_SETTLEMENT_TLS_CERT_FILE
FORGE_EXAMPLE_SETTLEMENT_TLS_KEY_FILE
FORGE_EXAMPLE_SETTLEMENT_TLS_CLIENT_CA_FILE
```

证书和 Token 摘要应由 Kubernetes Secret、CSI 或组织密码平台挂载，不能写入 Git。OTLP 使用主应用相同的 `FORGE_OTLP_*` 企业 CA/mTLS 配置。
